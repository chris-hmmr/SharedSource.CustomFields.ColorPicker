...
namespace Sitecore.SharedSource.CustomFields.ColorPicker
{
   /// <summary>
   /// Color Picker Field
   /// </summary>
   public class ColorPickerField : Sitecore.Shell.Applications.ContentEditor.Text, IContentField
   {
      ...
      //public bool ExplicitClassic
      //{
      //   get
      //   {
      //      var useClassic = WebUtil.ParseQueryString(Source)["useclassic"];

      //      if (String.IsNullOrEmpty(useClassic)) return false;

      //      return useClassic.Equals(Boolean.TrueString, StringComparison.InvariantCultureIgnoreCase);
      //   }
      //}
      ...
      protected void SetColorAdvanced(ClientPipelineArgs args)
      {
         ...
         {
            var url = new UrlString("/sitecore/shell/~/xaml/Sitecore.SharedSource.CustomFields.ColorPicker.ColorPickerDialog.aspx");
            url.Add("value", GetValue());
            SheerResponse.ShowModalDialog(url.ToString(), "325", "450", string.Empty, true);
            args.WaitForPostBack();
         }
      }
